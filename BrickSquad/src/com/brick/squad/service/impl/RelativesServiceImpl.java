...
public class RelativesServiceImpl implements RelativesService {
	...
	/*
	 * @Override public void userUpdateRelatives(
	 * RelativesAndAddressAndTypeExpand relativesAndAddressAndTypeExpand) throws
	 * Exception {
	 * 
	 * Address address = relativesAndAddressAndTypeExpand.getAddress();
	 * Relatives relatives = relativesAndAddressAndTypeExpand.getRelatives(); if
	 * (address.getId().equals("") || address.getId() == null) { //
	 * 如果没有插入地址过，那就先插入address addressMapper.insertAddress(address); //
	 * 地址插入成功后返回的ID存入relatives relatives.setAddressId(address.getId()); } else {
	 * // 有地址，则直接根据ID修改 addressMapper.updateAddressById(address); } if
	 * (relatives.getId().equals("") || relatives.getId() == null) { //
	 * 如果relativeID为空，则执行插入操作 relativesMapper.insertRelatives(relatives); } else
	...
	public void userDeleteRelativesById(String id) {
		...
		if (relatives != null) {
			if (relatives.getAddressId() != null) {
				// 先查询
				Address address = addressMapper.findAddressById(relatives
						.getAddressId());
				if (address != null) {
					// 再删除
	...
	public String usergetrelativesPagination(String id) throws Exception {
		...
		List<Relatives> relativesList = relativesMapper
				.selectRelativesByPerId(id);
		...
		if (relativesList != null) {
			for (Relatives relatives : relativesList) {
				...
				// 将亲属信息存在扩展类中
				relativesAndAddressAndTypeExpand
						.setRelativesPeopleName(relatives.getName());
				relativesAndAddressAndTypeExpand
						.setRelativesTelephone(relatives.getTelephone());
				relativesAndAddressAndTypeExpand.setRelatives(relatives);
				relativesAndAddressAndTypeExpand.setRelativesId(relatives
						.getId());
				// 如果亲属的地址ID不为空
				if (relatives.getAddressId() != null) {
					// 取得亲属的地址ID
					...
					if (address != null) {
						...
						String addressNameString = addressService
								.getAddressStringByAddress(address);
						// 把处理好的地址字符串放在封装类
						relativesAndAddressAndTypeExpand
								.setRelativesPeopleAddressName(addressNameString);
					}
				...
				if (relatives.getRelationshipId() != null) {
					...
					if (type != null) {
						...
						// 把处理好的亲属关系类型名存入封装类
						relativesAndAddressAndTypeExpand
								.setRelativesName(relationshipName);
					}

				}
				// 将处理好的亲属地址类型扩展类对象存在集合中
				relativesAndAddressAndTypeExpandsList
						.add(relativesAndAddressAndTypeExpand);
			}
		...
		String data = util
				.SplitPage(relativesAndAddressAndTypeExpandsList, row);
		return data;
	}
